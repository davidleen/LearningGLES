...
void main()     
{                            		
    vec3 newPosition = (uMMatrix * vec4(aPosition,1)).xyz; //根据总变换矩阵计算此次绘制此顶点位置
      vec3 normalTarget=aPosition+aNormal;   //向量目标点
     vec3 newNormal= (uMMatrix * vec4(normalTarget,1)).xyz-newPosition;   //转换后的向量法向量

      vTextureCoord = aTexCoor;//将接收的纹理坐标传递给片元着色器

       vec3 incident=newPosition-uCamera;
       v_incident=incident;
